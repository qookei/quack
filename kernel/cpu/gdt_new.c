...
typedef struct {
	tss_t tss;
	uint8_t io_bitmap[8196];
} __attribute__((packed)) tss_io_bitmap_t;

tss_io_bitmap_t tss_io;

void gdt_set_tss_stack(uint32_t stack) {
	tss_io.tss.esp0 = stack;
}

void gdt_load_io_bitmap(uint8_t *io_bitmap) {
	memcpy(tss_io.io_bitmap, io_bitmap, 8192);
}
...
void gdt_new_setup() {
	...
	gdt_arr[5] = create_descriptor((uint32_t)&(tss_io.tss), sizeof(tss_io), (0x4089));
