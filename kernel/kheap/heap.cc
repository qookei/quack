...
void heap_init() {
	_first 	= (memory_chunk *)0x1000;
	mem_top = 0x1000;
	void *page = (void *)pmm_alloc();
	map_page(page, _first, 0x3);
	_first->allocated = false;
	_first->size = 0x1000 - sizeof(memory_chunk);
	_first->prev = 0;
	_first->next = 0;
...
void *kmalloc(size_t size) {
	memory_chunk *res = 0;
	for (memory_chunk *chunk = _first; chunk != 0 && res == 0; chunk = chunk->next) {
